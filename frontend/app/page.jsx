...
function AuthenticatedDashboard({ user, profile, recentListings, loading }) {
  return (
...
function PublicHero({ user, loading, recentListings }) {
  return (
    <div className="relative flex flex-col min-h-screen overflow-hidden" style={marketBackground}>
      <div className="absolute inset-0 bg-black/45" />
      <div className="relative z-10">
        {/* Hero Section — renders immediately, never waits on auth state */}
        <motion.section
          initial="hidden"
          animate="visible"
        ...
        >
          ...
          <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-4">
            ...
            {/* Auth-dependent CTA: default to the guest action until auth resolves,
                then swap in-place once we know the real state. Never blocks paint. */}
            {!loading && user ? (
              <Link
                href="/listing/new"
        ...
        >
          ...
          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
            <Link
              href={!loading && user ? "/listing/new" : "/register"}
            ...
            >
              {!loading && user ? 'Post Your First Item' : 'Create Free Account'}
            </Link>
          </motion.div>
        </motion.section>
...
export default function LandingPage() {
  ...
  useEffect(() => {
    async function fetchRecent() {
      try {
        ...
        if (data?.listings) {
          setRecentListings(data.listings.slice(0, 8));
        }
      } catch (err) {
  ...
  // Once auth resolves, authenticated users see the dashboard. Until then —
  // and for anonymous visitors — we show the public marketing hero
  // immediately. The hero never blocks on authLoading anymore, which is
  // what was previously stalling LCP for every visitor, logged in or not.
  if (!authLoading && user) {
    return <AuthenticatedDashboard user={user} profile={profile} recentListings={recentListings} loading={loading} />;
  }

  return <PublicHero user={user} loading={authLoading} recentListings={recentListings} />;
}